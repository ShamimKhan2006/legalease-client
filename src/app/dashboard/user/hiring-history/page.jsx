...
export default function UserHiringHistory() {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      axios
        .get(
          `${process.env.NEXT_PUBLIC_URL}/user/hiring-history?email=${session.user.email}`,
        )
        .then((res) => {
          setHistory(res.data);
          setLoading(false);
  ...
  const totalPaid = history
    ...
    .reduce((sum, h) => sum + (h.lawyerFee || 0), 0);
    ...
    <div className="bg-[#0d0f14] rounded-2xl p-6">
          ...
          <tbody>
            {history.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="text-center py-10 text-[#40485a] text-sm"
                >
              ...
              history.map((hire) => (
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    <div className="text-[13px] font-semibold text-[#e0e3ec]">
                      {hire.lawyerName || "—"}
                    </div>
                    <div className="text-[11px] text-[#506070] mt-0.5">
                      {hire.specialization || ""}
                    </div>
                  </td>
                  ...
                  {/* Fee */}
                  <td className="px-3 py-3">
                    <span className="font-mono text-[12px] text-[#7ab0e0]">
                      ${(hire.lawyerFee || 0).toLocaleString()}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-3">
                    <span
                      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${statusStyle[hire.status] ?? statusStyle.pending}`}
                    >
                  ...
                  <td className="px-3 py-3">
                    ...
                    {hire.paymentStatus === "paid" && (
                      <span className="font-mono text-[11px] text-[#3daa60] tracking-wider">
                        ● Paid
                      </span>
                    )}
                    {hire.status === "rejected" && (
      ...
      {totalPaid > 0 && (
        <div className="mt-4 pt-3 border-t border-[#13171f] flex justify-between items-center">
          <span className="text-[11px] text-[#40485a] uppercase tracking-widest">
            Total paid
          </span>
