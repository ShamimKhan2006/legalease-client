...
// import { Menu, X, Scale, LayoutDashboard, Sun, Moon, ToggleLeft } from "lucide-react";
...
//   const effectiveScrolled = bgToggle ? true : scrolled;
...
//                   bgToggle
//                     ? "dark:bg-indigo-500/20 dark:border-indigo-400/40 dark:text-indigo-400 bg-indigo-100 border border-indigo-300 text-indigo-600"
//                     : "dark:bg-white/5 dark:border-white/10 dark:text-slate-400 dark:hover:text-white bg-black/5 border border-black/10 text-slate-500 hover:text-slate-900 hover:bg-black/8"
//                 }`}
//                 title={bgToggle ? "Remove navbar background" : "Add navbar background"}
//               >
//                 <ToggleLeft size={18} />
//               </button>
  ...
  return (
    <div className={`sticky top-0 z-50 w-full transition-all duration-500 ${scrolled ? "px-4 pt-3 pb-2" : "px-0 pt-0 pb-0"}`}>
      <header
        className={`transition-all duration-500 ${
          scrolled
            ? "max-w-7xl mx-auto rounded-2xl backdrop-blur-xl shadow-2xl border bg-slate-900/95 shadow-black/30 border-white/5"
            : "bg-slate-950 border-white/5 border-b"
        }`}
      >
        <div className="max-w-7xl mx-auto px-5 md:px-8 ">
          <div className="flex h-16 items-center justify-between">
            ...
            {/* Desktop Nav */}
            <nav className="hidden md:flex items-center gap-1 p-1 rounded-full bg-white/5 border border-white/8">
              {navLinks.map((link) => {
                const isActive = pathname === link.href;
                return (
                  <Link
            ...
            <div className="hidden md:flex items-center gap-2">
              {isPending ? (
                <div className="w-8 h-8 rounded-full bg-white/10 animate-pulse bg-textground" />
              ) : user ? (
                <CustomTrigger handleLogout={handleLogout} />
              ) : (
                <div className="flex items-center gap-2">
            ...
            <div className="md:hidden flex items-center gap-2">
              <button
                onClick={() => setMenuOpen(!menuOpen)}
                className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 text-slate-400 hover:text-white transition-all active:scale-95"
              >
                {menuOpen ? <X size={16} /> : <Menu size={16} />}
              </button>
            </div>
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className="md:hidden border-white/5 border-t px-5 py-4 space-y-3">
            <div className="space-y-1">
              {navLinks.map((link) => {
                const isActive = pathname === link.href;
                return (
            ...
            <div className="pt-3 border-white/5 border-t">
              {isPending ? (
                <div className="h-10 rounded-xl bg-white/5 animate-pulse" />
              ) : user ? (
                <div className="flex items-center justify-between px-4 py-2 rounded-xl bg-white/5 border border-white/8">
                  <div>
                ...
                <div className="flex flex-col gap-2">
                  <Link
                    href="/login"
                    onClick={() => setMenuOpen(false)}
                    className="w-full text-center px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-300 bg-white/5 border border-white/10 hover:bg-black/8 transition-all"
                  >
                    Login
                  </Link>
                  <Link
